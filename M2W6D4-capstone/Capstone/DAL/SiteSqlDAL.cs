...
namespace Capstone.DAL
{
	public class SiteSqlDAL
	{
        ...
        public List<Site> GetAvailableSitesFromPark(DateTime toDate, DateTime fromDate, int parkId)
        {
            List<Site> output = new List<Site>();
            // will return a list of availble reservations based on campgroundId, toDate, and fromDate.
            // We will have a SQL command which will join site with reservations on site_id and check the to and from date and
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand(
                        @"SELECT TOP 5 site.site_id, site.site_number, site.max_occupancy, site.accessible, site.max_rv_length, site.utilities, DATEDIFF(d,@fromDate, @toDate) * campground.daily_fee AS total_cost
                        FROM site  
                        JOIN campground ON campground.campground_id = site.campground_id
